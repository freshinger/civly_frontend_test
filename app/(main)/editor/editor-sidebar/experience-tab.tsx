      ...
      .map((r) => {
        ...
        return {
          ...
          sortKey: [
            ...
            Number.isNaN(endTs) ? -1 : endTs, // then by end desc
            Number.isNaN(startTs) ? -1 : startTs, // then by start desc
            -r.idx, // stable fallback
          ] as const,
        };
      })
      .sort((a, b) => {
...
function ExperienceRow({
  ...
  const handleDisplayPropsChange = (
    field: keyof ExperienceDisplayProps,
    value: string,
    placeholder: string
  ) => {
    ...
    <Section className={cn("gap-2 bg-card", className)}>
      ...
      >
        ...
        <div className="h-7 flex gap-4 items-center">
          ...
          />
          <Label htmlFor={id("currentlyWorkingHere")}>
            Currently Working Here
          </Label>
        </div>
